...
description: "A class used to build the SQL statements for the MySqlConnector-based MariaDb data provider."
...
The parameterless constructor resolves `dbSetting` via `DbSettingMapper.Get<MariaDbConnection>()`, so it can only be used after [MariaDbBootstrap](/class/mariadb/mariadbconnector/mariadbbootstrap) has run.
