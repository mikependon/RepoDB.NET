...
- `ClickHouseBootstrap` / `ClickHouseGlobalConfiguration.UseClickHouse()` — initializes RepoDB for use with `ClickHouseConnection` (`GlobalConfiguration.Setup().UseClickHouse()`), mapping the `ClickHouseDbSetting`, `ClickHouseDbHelper`, and `ClickHouseStatementBuilder` to every `ClickHouseConnection`. An overload, `UseClickHouse(IDbSetting)`, accepts a custom setting instance in place of the default `ClickHouseDbSetting` — see [RepoDb.ClickHouse.BulkOperations](/release/clickhousebulk)'s `ClickHouseBulkDbSetting`.
