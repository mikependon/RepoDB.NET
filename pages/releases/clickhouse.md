...
- [IClickHouseDbSetting](/interface/clickhouse/iclickhousedbsetting) — extends [IDbSetting](/interface/idbsetting) with an `IsWaitForMutationsEnabled` property, consulted by `BulkMerge`/`BulkUpdate`/`BulkDelete` in [RepoDb.ClickHouse.BulkOperations](/release/clickhousebulk) to decide whether to block until their `ALTER TABLE` mutation completes; the plain `Update`/`UpdateAll`/`Delete`/`DeleteAll` operations never read it. Implemented by `ClickHouseDbSetting`.
