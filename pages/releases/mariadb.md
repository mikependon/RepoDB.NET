...
## RepoDb.MariaDb (v0.0.1)

Released: September 8, 2026

First general availability (non-preview) release of the MariaDB provider, promoting the `v0.0.1-alpha1` preview below to stable. No functional changes since `v0.0.1-alpha1` — see that entry for the full feature set and known limitations.
...
- [QueryMultiple](/operation/executequerymultiple)/`QueryMultipleAsync` run as a single combined, multi-statement command text (`"SELECT ... T1; SELECT ... T2"`) — no per-type round trip.
- Targets .NET Standard 2.0, .NET 8, .NET 9, and .NET 10.

### Known limitations (v1)

- Install only one of `RepoDb.MariaDb` or [RepoDb.MariaDbConnector](/release/mariadbconnector) per project — both map against the identically-named `MariaDbConnection` type, so referencing both lets the second package's mappings silently overwrite the first's. See [Get Started for MariaDB](/tutorial/get-started-mariadb) for details.
...
- No native GUID/`UNIQUEIDENTIFIER` type — map a `Guid` property as `string`/`byte[]`, or register a custom [IPropertyHandler](/interface/ipropertyhandler).
