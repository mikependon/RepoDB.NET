...
- Every bulk call creates its own physical staging table (`CREATE TABLE ... DEFINITION ONLY`) and drops it once the call completes, rather than reusing one across calls. Since `CREATE TABLE`/`DROP TABLE` are DDL and commonly force a commit boundary, this happens on every single call, not just the first.
...
- No `Unspecified` identity-behavior state — `Db2BulkImportIdentityBehavior` defaults straight to `KeepIdentity`.
