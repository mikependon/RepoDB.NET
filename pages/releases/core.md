...
- Added the [QueryMultipleSeparator](/class/basedbsetting) property to the [IDbSetting](/interface/idbsetting) interface (and its [BaseDbSetting](/class/basedbsetting) implementation), giving each provider explicit control over the string used to join the individual command texts generated by the [QueryMultiple](/operation/querymultiple) and `QueryMultipleAsync` operations. It defaults to a single space (`" "`), which suits providers (SQL Server, MySQL, PostgreSQL) whose statement builder already terminates every generated statement with its own trailing separator. Providers whose driver rejects a trailing statement terminator on an otherwise-standalone command can instead override it with their own inter-statement separator (e.g. `"; "`) so the combined command text remains valid. This groundwork is part of the upcoming IBM Db2 provider support. [#1241](https://github.com/mikependon/RepoDB/issues/1241)
