...
- Added the [RequiresDbTypeBeforeValue](/class/basedbsetting) property to the [IDbSetting](/interface/idbsetting) interface (and its [BaseDbSetting](/class/basedbsetting) implementation). Defaults to `false`; when `true`, `DbParameter.DbType` is assigned (inferred from the value's CLR type, if not explicitly given) before `DbParameter.Value` is set, instead of after. Some providers (e.g. Vertica) lazily initialize internal parameter state inside the `DbType` setter and throw if `Value` is assigned first on a parameter fresh off `DbCommand.CreateParameter()`.
- Added the [SkipsUnreferencedParameters](/class/basedbsetting) property to the [IDbSetting](/interface/idbsetting) interface (and its [BaseDbSetting](/class/basedbsetting) implementation). Defaults to `false`; when `true`, a bound parameter with no corresponding placeholder in the generated `DbCommand.CommandText` is skipped instead of bound. A null-valued equality filter (e.g. `WHERE "Id" = @Id` with a `null` value) is rendered by [QueryField](/class/queryfield) as the literal `"Id" IS NULL` with no `@Id` placeholder at all, yet the parameter is still normally bound — most providers silently tolerate the unused parameter, but a strict provider rejects the whole command over it.
- Added the [MaxParameterCount](/class/basedbsetting) property to the [IDbSetting](/interface/idbsetting) interface (and its [BaseDbSetting](/class/basedbsetting) implementation), capping the number of parameters/members the current DB provider allows in a single generated command text — most directly, the number of values a single `WHERE column IN (...)` clause can hold. Operations that batch a large key list into that shape (e.g. `DeleteAll(keys)`) now split it into chunks no larger than this value before generating each chunk's command text. Defaults to `2098` (SQL Server's ~2100 parameter ceiling, minus a small safety margin); override it for a provider with a lower hard limit (e.g. Firebird's DSQL parser rejects an `IN (...)` list past 1500 members with "Implementation limit exceeded").
- Added the [MultiStatementSeparator](/class/basedbsetting) property to the [IDbSetting](/interface/idbsetting) interface (and its [BaseDbSetting](/class/basedbsetting) implementation), giving each provider explicit control over the string used to join the individual command texts generated by the [QueryMultiple](/operation/querymultiple) and `QueryMultipleAsync` operations. It defaults to `";"`. Providers whose driver rejects a trailing statement terminator on an otherwise-standalone command can override it with their own inter-statement separator so the combined command text remains valid. This groundwork is part of the upcoming IBM Db2 provider support. [#1241](https://github.com/mikependon/RepoDB/issues/1241)
- Added the [SqlTextParameterPrefix](/class/basedbsetting) property to the [IDbSetting](/interface/idbsetting) interface (and its [BaseDbSetting](/class/basedbsetting) implementation), giving each provider explicit control over the parameter prefix used when a parameter placeholder is embedded directly in a raw/text SQL statement, as opposed to when it is bound via a `DbParameter` object.
- Added the [IsTransactionSupported](/class/basedbsetting) property to the [IDbSetting](/interface/idbsetting) interface (and its [BaseDbSetting](/class/basedbsetting) implementation), giving each provider explicit control over whether the underlying ADO.NET driver supports transaction objects.
