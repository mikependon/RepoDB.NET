...
- [SapHanaBulkDbSetting](/class/saphana/saphanabulkdbsetting) — an opt-in [SapHanaDbSetting](/class/saphana/saphanadbsetting) subclass exposing `WriteToServerExecution` (via [SapHanaWriteToServerExecution](/enumeration/saphana/saphanawritetoserverexecution)), letting the `Async` overloads fall back to the synchronous `HanaBulkCopy` path instead of [SapHanaCommandBatcher](/class/saphana/saphanacommandbatcher). Not registered by default — register it explicitly via [DbSettingMapper](/mapper/dbsettingmapper) to opt in.
