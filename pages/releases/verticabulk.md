...
## RepoDb.Vertica.BulkOperations (v1.0.0)

Released: September 8, 2026

First general availability (non-preview) release of the Vertica Bulk Operations package, promoting the preview below to stable `v1.0.0`. No functional changes since the preview — see that entry for the full feature set and known limitations.
...
- `BulkCopyTimeout` is accepted on every operation, but `VerticaCopyStream` has no timeout-equivalent property to apply it to — the argument currently has no effect.
...
- Every bulk call against a table it hasn't seen before creates its own uniquely-named staging table (`CREATE TABLE`/`CREATE GLOBAL TEMPORARY TABLE`) and drops it once the call completes, rather than reusing one across calls.
