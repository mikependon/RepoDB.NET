...
- [VerticaBootstrap](/class/vertica/verticabootstrap) / [VerticaConfiguration](/class/vertica/verticaconfiguration)`.UseVertica(useInvariantCulture: false)` — initializes RepoDB for use with `VerticaConnection` (`GlobalConfiguration.Setup().UseVertica()`), mapping the [VerticaDbSetting](/class/vertica/verticadbsetting), [VerticaDbHelper](/class/vertica/verticadbhelper), and [VerticaStatementBuilder](/class/vertica/verticastatementbuilder) to every `VerticaConnection`. Pass `useInvariantCulture: true` to work around `Vertica.Data` formatting date-like values using the ambient thread culture (see Known limitations below).
...
- `useInvariantCulture: true` on `UseVertica()` forces `CultureInfo.CurrentCulture` to `CultureInfo.InvariantCulture` process-wide, not just for the current connection — enabling it affects every other culture-sensitive code path running in the same process.
