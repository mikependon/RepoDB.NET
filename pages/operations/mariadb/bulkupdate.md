...
sidebar: operations
title: "BulkUpdate"
permalink: /operation/mariadb/bulkupdate
tags: [repodb, tutorial, bulkupdate, orm, hybrid-orm, mariadb]
parent: "MariaDb"
...
This method updates existing rows in the database in bulk, matched by the defined qualifiers. It is supported for [RepoDb.MariaDb.BulkOperations](https://www.nuget.org/packages/RepoDb.MariaDb.BulkOperations), targeting the [MySql.Data](https://www.nuget.org/packages/MySql.Data)-based driver.

{: .note }
> This page documents the `RepoDb.MariaDb` (`MySql.Data`-based) arguments and examples. For the `MySqlConnector`-based implementation, see [BulkUpdate (MariaDbConnector)](/operation/mariadbconnector/bulkupdate); for MySQL, see [BulkUpdate (MySQL)](/operation/mysql/bulkupdate).
...
Use this method to update rows at high speed. It leverages this package's internal `MariaDbBulkCopy` class (`LOAD DATA LOCAL INFILE`-based, see [Operations (MariaDb)](/operation/mariadb)).

For updating 1,000 or more rows, prefer this method over [UpdateAll](/operation/updateall).

A pseudo (staging) table is created for the call. The library writes to it via its internal `MariaDbBulkCopy`, then cascades the changes to the target table via a single `UPDATE ... INNER JOIN` statement — unlike [BulkMerge](/operation/mariadb/bulkmerge), staged rows with no matching target row are left as-is, not inserted. See [Operations (MariaDb)](/operation/mariadb) for the underlying mechanics.
...
An equivalent [BulkUpdateAsync](/operation/mariadb/bulkupdate) method is also available.
