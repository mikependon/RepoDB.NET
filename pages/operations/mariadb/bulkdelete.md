...
sidebar: operations
title: "BulkDelete"
permalink: /operation/mariadb/bulkdelete
tags: [repodb, tutorial, bulkdelete, orm, hybrid-orm, mariadb]
parent: "MariaDb"
...
This method deletes rows from the database in bulk, matched by the defined qualifiers. It is supported for [RepoDb.MariaDb.BulkOperations](https://www.nuget.org/packages/RepoDb.MariaDb.BulkOperations), targeting the [MySql.Data](https://www.nuget.org/packages/MySql.Data)-based driver. This package also has a dedicated [BulkDeleteByKey](/operation/mariadb/bulkdeletebykey) operation for deleting by primary key.

{: .note }
> This page documents the `RepoDb.MariaDb` (`MySql.Data`-based) arguments and examples. For the `MySqlConnector`-based implementation, see [BulkDelete (MariaDbConnector)](/operation/mariadbconnector/bulkdelete); for MySQL, see [BulkDelete (MySQL)](/operation/mysql/bulkdelete).
...
Use this method to delete rows at high speed. It leverages this package's internal `MariaDbBulkCopy` class (`LOAD DATA LOCAL INFILE`-based, see [Operations (MariaDb)](/operation/mariadb)).

For deleting 1,000 or more rows, prefer this method over [DeleteAll](/operation/deleteall).

A pseudo (staging) table is created for the call. The library writes to it via its internal `MariaDbBulkCopy`, then cascades the deletions to the target table via a `DELETE ... INNER JOIN` matched on the qualifiers — see [Operations (MariaDb)](/operation/mariadb) for the underlying mechanics.
...
An equivalent [BulkDeleteAsync](/operation/mariadb/bulkdelete) method is also available.
